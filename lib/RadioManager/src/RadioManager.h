...
public:
    ...
    bool setPairedAddr(String& address, uint8_t channel, uint8_t* publicKey, uint8_t* sharedKey);
    void clearPairedAddr(uint8_t channel);
    bool clearPairedUID(String& uid);
    String getPairedAddrList();
    bool setPairedAddrList(const String& addrList);

    // Encryption functions
    bool setPersonalKeys(const Bytes& publicKey, const Bytes& privateKey);
    bool setPairedKeys(uint8_t channel, const Bytes& publicKey, const Bytes& sharedKey);
