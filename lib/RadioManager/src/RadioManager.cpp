 ...
 * @param sharedKey Pointer to shared key (32 bytes)
 * @return true if the operation was successful, false otherwise
 */
bool RadioManager::setPairedAddr(String& address, uint8_t channel, uint8_t* publicKey, uint8_t* sharedKey) {
    if (channel >= 0 && channel < MAX_CHANNELS) {
        clearPairedAddr(channel);
        pairedDevices[channel].addr = address;
            ...
            if (sentPubKey && !gotAck && radio.available()) {
                ...
                if (checkValidAddr(receivedAddr)) {
                    ...
                    // Otherwise, pair the received address on the available channel if we have room
                        ...
                        setPairedAddr(receivedAddr, pairingChannel, tempPublicKey, tempSharedKey);
...
/**
 * @brief Gets the list of paired Addrs as a string
 * 
 * @return A string containing the list of paired Addrs, separated by commas. 
 *         "0" represents an unpaired channel.
 */
String RadioManager::getPairedAddrList() {
    String addrList = "";
    for (int i = 0; i < MAX_CHANNELS; i++) {
        if (pairedDevices[i].addr.isEmpty()) {
            addrList += "0";
        } else {
            addrList += pairedDevices[i].addr;
        }
        if (i < MAX_CHANNELS - 1) {
            addrList += ",";
        }
 ...
 * 
 * @param addrList A string containing the list of Addrs to set, separated by commas.
 *                "0" represents an unpaired channel.
 * @return true if the operation was successful, false otherwise
 */
bool RadioManager::setPairedAddrList(const String& addrList) {
    String addrArray[MAX_CHANNELS];
    int addrCount = 0;
    int startIndex = 0;
    int commaIndex;

    // Parse the addrList string
    while ((commaIndex = addrList.indexOf(',', startIndex)) != -1 && addrCount < MAX_CHANNELS) {
        addrArray[addrCount++] = addrList.substring(startIndex, commaIndex);
        startIndex = commaIndex + 1;
    }
    if (addrCount < MAX_CHANNELS) {
        addrArray[addrCount++] = addrList.substring(startIndex);
    }

    // Check if we have the correct number of Addrs
    if (addrCount != MAX_CHANNELS) {
        return false;
    }
 ...
 * 
 * @param publicKey Reference to vector containing pub key
 * @param privateKey Reference to vector containing private key
 * 
 ...
 * 
 * @param publicKey Reference to vector to store the public key
 * @param privateKey Reference to vector to store the private key
 * 
 ...
 * 
 */
bool RadioManager::setPairedKeys(uint8_t channel, const Bytes& publicKey, const Bytes& sharedKey) {
    if (channel < MAX_CHANNELS) {
        if (publicKey.size() == KEY_SIZE && sharedKey.size() == KEY_SIZE) {
            memcpy(this->pairedDevices[channel].publicKey, publicKey.data(), KEY_SIZE);
            memcpy(this->pairedDevices[channel].sharedKey, sharedKey.data(), KEY_SIZE);
